 ...
 */
public class CloudFeePayActivity extends BaseActivity implements OnClickListener {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_submit:
                if (!(payPreVo == null)) {
                    if (payType == -1) {
                        Toast.makeText(CloudFeePayActivity.this, "请选择支付类型", Toast.LENGTH_SHORT).show();
                    } else {
                        if (payType == Constants.PAY_TYPE_BANK || payType == Constants.PAY_TYPE_SUYY || payType == Constants.PAY_TYPE_ALIPAY) {//银联，苏州银行
                            Intent intent = new Intent(CloudFeePayActivity.this, SZBankActivity.class);
...
//                            intent.putExtra("payChannel", payType == Constants.PAY_TYPE_BANK ? "6006" : "6003");
                    ...
                    Toast.makeText(CloudFeePayActivity.this, "预支付数据有误或还未获取到", Toast.LENGTH_SHORT).show();
                }
                break;
        }
    }
            ...
            if (null != result) {
                if (result.statue == Statue.SUCCESS) {
                    if (null != result.data) {
                        ...
                        Toast.makeText(CloudFeePayActivity.this, "获取支付信息失败", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    result.showToast(CloudFeePayActivity.this);
                }
            } else {
                Toast.makeText(CloudFeePayActivity.this, "获取支付信息失败", Toast.LENGTH_SHORT).show();
            }
...
//                System.out.println("payInfo:" + payInfo);
                Runnable payRunnable = new Runnable() {
                    ...
                    public void run() {
                        // 构造PayTask 对象
                        PayTask alipay = new PayTask(CloudFeePayActivity.this);
                        // 调用支付接口，获取支付结果
            ...
            switch (msg.what) {
                case Constants.PAY_TYPE_ALIPAY://支付宝支付
                {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        Toast.makeText(CloudFeePayActivity.this, "支付成功", Toast.LENGTH_SHORT).show();
                        Intent intent = new Intent();
                        ...
                        if (TextUtils.equals(resultStatus, "8000")) {
                            Toast.makeText(CloudFeePayActivity.this, "支付结果确认中", Toast.LENGTH_SHORT).show();
                        } else {
                            // 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
                            Toast.makeText(CloudFeePayActivity.this, payResult.getMemo(), Toast.LENGTH_SHORT).show();
                        }
                    }
                    break;
                }
    ...
    private class GetPrePayTask extends AsyncTask<Void, Void, ResultModel<ArrayList<PayPreVo>>> {
        ...
        protected ResultModel<ArrayList<PayPreVo>> doInBackground(Void... params) {
            ...
            return HttpApi.getInstance().parserArray(PayPreVo.class, "auth/remote/cloudClinic/getClinicPayBudget", new BsoftNameValuePair("sbxhs", getFeeSbxhs()));
