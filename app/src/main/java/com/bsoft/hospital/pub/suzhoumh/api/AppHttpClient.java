...
//	};
	...
	public String doHttpPostConsultPic(String url, String[] sysfiles,
			ArrayList<BsoftNameValuePair> nameValuePairs) throws Exception {

		System.out.println("doHttpPostEhr sysfiles : " + sysfiles);

		HttpPost httpPost = new HttpPost(url);

		MultipartEntity multipartEntity = new MultipartEntity();
		if (null != sysfiles && sysfiles.length > 0) {
			for (int i = 0; i < sysfiles.length; i++) {
				if (!StringUtil.isEmpty(sysfiles[i])) {
					multipartEntity.addPart("sysfiles", new FileBody(new File(
							sysfiles[i])));
					System.out.println("sysfiles " + sysfiles[i]);
				}
			}
		}
			...
			for (int i = 0; i < nameValuePairs.size(); i++) {
				...
				/*multipartEntity.addPart(nameValuePairs.get(i).getName(),
						new StringBody(nameValuePairs.get(i).getValue(),
								Charset.forName("UTF-8")));*/
				multipartEntity.addPart(nameValuePairs.get(i).getName(),
						new StringBody(nameValuePairs.get(i).getValue(),Charset.forName("UTF-8")));
			}
		}
